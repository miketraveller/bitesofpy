...
def get_users_for_shell(passwd_output: str = PASSWD_OUTPUT,
    ...
    """
    lines = [line for line in passwd_output.split("\n")]
    # for line in lines:
    #     user = line[:line.find(':')]
    #     shell = line[line.rfind(':'):]

    op = [(line[:line.find(":")], line[line.rfind('/')+1:]) for line in lines]
    return [entry[0] for entry in op if grep_shell in entry[1]]

    op = [(line[:line.find(":")], line[line.rfind('\\')+1:]) for line in lines]
    return sorted([entry[0] for entry in op if DEFAULT_SHELL in entry[1]])