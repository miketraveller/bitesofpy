...
def get_movies(string):
    ...
    for movie in tree.iter('movie'):
        movie_dict = movie.attrib
        # print(movie_dict['title'])
        yield movie_dict['title']

def get_movie_longest_runtime(string):
    ...
    for movie in tree.iter('movie'):
        movie_dict = movie.attrib
        runtime[movie_dict['title']] = movie_dict['runtime']
    return sorted(runtime, key=runtime.get, reverse=True)[0]
